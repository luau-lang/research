...
\subsection{Semantic subtyping for \anon{Luau}}
...
\end{itemize}
To give a semantic subtying, we first declare the domain $\mathcal{D}$
...
\item \emph{error values} $\ERROR$,
  witnessing a error-suppresing type,
  and
...
\item $\TRUE$ and $\FALSE$ and $\ERROR$ are values in the error-suppresing type $\BOOLEAN \cup \ERROR$,
  and 
\item $(\TRUE) \mapsto (\FALSE)$ is a value in the function type $\BOOLEAN \fun \BOOLEAN$.

\end{itemize}
Scalar and error-suppressing values are relatively straighforward, but
functions are trickier. The case where a type-correctness argument is
...
supplied, in which case there is no guarentee what is returned, for example:
\[
  ((5) \mapsto (37)) \in \sem{\BOOLEAN \fun \BOOLEAN}
\]
The guarantee applies when a type-correct argument is provided:
\[
...
\begin{itemize}
\item \anon{Luau} allows functions to be called without any arguemts
  (modeled in this sematics as $() \mapsto r$)
\item \anon{Luau} allows functions to check arguments,
  (modeled in this sematics as $a \mapsto \CHECK$), and
\item \anon{Luau}, as is common in most functional lanuages, allows functions to diverge
  (modeled in this sematics as $a \mapsto \DIVERGE$).
\end{itemize}
