...
Subtype relations between finite type packs is simple and obvious: A type pack $\overline{T}$ is a subtype of another $\overline{U}$ if their lengths are exactly equal and if each type in $\overline{T}$ is a subtype of the corresponding type from $\overline{U}$.  There is no subtyping relation at all between finite packs of inequal length.
