...
\[
  ...
  T_j = \STRING? \quad
\]
vvbut there is no appropriate $k$. Fortunately, we can extend $F$
...
\begin{proof}
  ...
  \[
    F = (S_1 \fun T_1) \cap \cdots \cap (S_{m} \fun T_{m})
  \]
  Let $n$ be $2^m$ as the powerset of $1..m$.
  The extenson of $F$ has:
  \[
    (S_{i+j} \fun T_{i+j})
      = ((S_{i} \cup S_{j}) \fun (T_{i} \cup T_{j}))
  \]
  To check that is saturared, for $i$ and $j$, then pick $k = i+j$
