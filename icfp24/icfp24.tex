...
\begin{proof}
  ...
  \item $S \cap T$: By indication $S$ and $T$. We use Lamma~\ref{lem:funnord}
    to normalize the function, and normalizes a union of scaler types is staightforwrd.
  \item $S \cup T$: By indication $S$ and $T$. We use Lamma~\ref{lem:funnord}
    to normalize the function, and normalizes a interset of scaler types is staightforwrd.
  \end{itemize}
  This is an induction where the only tricky case is functions, which is given by Lamma~\ref{lem:funnord}.
\end{proof}
...
\caption{The source $\SRC(F)$
  and the application $\APP(F, V)$ of a normalized fuction type $F$ and argument $V$}
\label{fig:ntypes}
...
\]
and then we have cases for functional normalize types (under a conditioal \emph{satufared} discissed later):
\[\begin{array}{r@{\,}c@{\,}ll}
  (S \fun T) & \not<: & F & \mbox{ when } S \not<: \SRC(F) \\
