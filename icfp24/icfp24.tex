...
\begin{proof}
  If $F'$ is $\NEVER$ then just return $F$ is $F''$.
  If $F''$ is $\NEVER$ then just return $F$ is $F'$.
  Otherwise, we first distribution union through introspection:
  \[
  ...
  \]
  and where $m$ is $m' \times m''$.
  Now use Lemma~\ref{lem:union-though-fun} to get:
  \[
    ((S'_{i} \fun T'_{i}) \cup (S''_{i} \fun T''_{i}))
  \]
  has the same
  \[
    (S'_{i} \cap S''_{i}) \fun (T'_{i}) \cup T''_{i}))
  \]
  as required.
\end{proof}
