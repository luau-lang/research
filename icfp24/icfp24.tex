...
\begin{proof}\mbox{}
  ...
  there is a overload $F <: (S_i \fun T_i)$ where $S <: S_i$
  and $T_i <: T$.
  \begin{itemize}
  ...
  \item[($\Rightarrow$)]
    We hypothesis
    $F <: (S \fun T)$.
    Lemma~\ref{lemma:elim-fun-variant} $S <: S_i$ and $T_i <: T$
    and so by transivity
    $S <: \SRC(F)$
    and $\APP(F, S) <: T$
  ...
  \item[($\Leftarrow$)] 
    ...
    $S <: \SRC(F)$ and
    $\APP(F) <: T$.
    By Lemma~\ref{lemma:intro-fun-variant} $(S_i \fun T_i) <: (S \fun T)$
    and so by transivity $F <: (S \fun T)$.

  \end{itemize}
  Note Lemma~\ref{lemma:elim-fun-variant}
  depends on pragmatic semantic subtyping.
\end{proof}

\subsection{Algorithm for Deciding Subtypes of Normalization Types}
\label{subsec:algth}

We now have an algorithm for deciding subtypig.

\begin{theorem}
  We have decision for subtyping.
\end{theorem}

\begin{proof}
  We prove by induction on the depth of types,
  which means that $(S \fun T') <: (S' \fun T)$ can use $S <: S'$ and $T <: T'$ by induction.
  ...
  \item $S \not<: \NEVER$ when $S$ is a function or scalar.

  \item $S <: \UNKNOWN$.

  \item $\UNKNOWN \not<: T$ when $T$ is a function or scalar.
  
  \item for scalars $s$ and $t$, if $s = t$ then $s <: t$ otherwise $s \not<: t$.

  \item for scalar $s$ and functions $(S \fun T)$, $s \not<: (S \fun T)$ and 
     $(S \fun T) \not<: s$.

  \item for functions $(S_1 \fun T_1)$ and $(S_2 \fun T_2)$
    \begin{itemize}
    \item if $S_2 \not<: S_1$ then $(S_1 \fun T_1) \not<: (S_2 \fun T_2)$,
    \item if $T_1 \not<: T_2$ then $(S_1 \fun T_1) \not<: (S_2 \fun T_2)$, and
    \item if $S_2 <: S_1$ and $T_1 <: T_2$ then $(S_1 \fun T_1) <: (S_2 \fun T_2)$.
   \end{itemize}
    
  \item comparing $S$ and $(T_1 \cap T_2)$:
    \begin{itemize}
    \item if $S \not<: T_1$ then $S \not<: (T_1 \cap T_2)$,
    \item if $S \not<: T_2$ then $S \not<: (T_1 \cap T_2)$, and
    \item if $S <: T_1$ and $S <: T_2$ then $S <: (T_1 \cap T_2)$.
    \end{itemize}

  \item comparing $(S_1 \cup S_2)$ and $T$ as:
    \begin{itemize}
    \item if $S_1 \not<: T$ then $(S_1 \cup S_2) \not<: T$,
    \item if $S_2 \not<: T$ then $(S_1 \cup S_2) \not<: T$, and
    \item if $S_1 <: T$ and $S_2 <: T$ then $(S_1 \cup S_2) <: T$.
    \end{itemize}

  \end{itemize}
  Those are the cases, but this cases leaves where normaized cases,
  and saturated types for fuctions.
  The cases where normalization is used:
  \begin{itemize}

  \item comparing $S$ and $(T_1 \cup T_2)$, and

  \item comparing $(S_1 \cap S_2)$ and $T$.

  \end{itemize}
  Cases:
  \begin{itemize}

  \item compaing $N$ and $\NEVER$:
   \begin{itemize}
    \item if $N$ is $F \cup s_1 \cup\cdots\cup s_n$, then $N \not<: \NEVER$, and
    \item if $N$ is $\NEVER$, then $N <: \NEVER$.
   \end{itemize}

  \item compaing $\UNKNOWN$ and $N$
   \begin{itemize}
   \item if $N$ is $\NEVER$, then $\UNKNOWN \not<: N$,
   \item if $N$ is $F \cup s_1 \cup\cdots\cup s_n$, and there is missing $s$ then $\UNKNOWN \not<: N$,
   \item if $N$ is $F \cup s_1 \cup\cdots\cup s_n$, and $F \not<: \FUNCTION$ then $\UNKNOWN \not<: N$, and
   \item if $N$ is $F \cup s_1 \cup\cdots\cup s_n$, and $F <: \FUNCTION$ and ever scalar
     mentioned then $\UNKNOWN <: N$..
   \end{itemize}

  \item compaing scalar $s$ and $N$
   \begin{itemize}
   \item if $N$ is $\NEVER$, then $s \not<: N$,
   \item if $N$ is $F \cup s_1 \cup\cdots\cup s_n$, and there no $s_i = s$, then $s \not<: N$, and
   \item if $N$ is $F \cup s_1 \cup\cdots\cup s_n$, and there is $s_i = s$, then $s <: N$.
   \end{itemize}

  \item compaing $N$ and $(S \fun T)$
...
\subsection{Function features: Oversaturation}
...
It is tempting to introduce a subtyping rule between functions of mismatched arities (eg $() \rightarrow () <: <T>(T) \rightarrow ()$), but this leads to soundness issues:
...
In the example above, we infer $print_string : (string?, ...unknown) \rightarrow ()$.

\subsection{Function features: Undersaturation}

In Luau, as in Lua, a function can also be called with fewer arguments than it has parameters.  At runtime, this has same effect as passing $nil$ for those extra parameters.

A very common practice is to use this mechanism to effect optional parameters.
...
f(0, true) -- Prints "true"
f(0, nil)  -- Does not print anything
f(0)       -- Same as f(nil)
\end{verbatim}

Luau considers an argument to be optional if its type is a subtype of $nil$.  For instance, $f$ above could be explicitly annotated like so:

\begin{verbatim}
function f<T>(x: T, y: boolean?): T
\end{verbatim}

Modelling this in the type system is a challenge.  Doing so via a subtyping relation a la $(T?) -> () <: () -> ()$ creates a soundness problem:

TODO remember what the issue was here!

\subsection{Function features: variadic functions}
...
\begin{verbatim}
function apply_five(cb: (number) -> ())
    cb(5)
end

apply_five(function() end)
\end{verbatim}

Luau models functions as maps from one \textit{type pack} to another.  A type pack is an ordered set of 0 or more types.  Like types, type packs can be concrete or generic.  They can also be finite or infinite.

Subtype relations between finite type packs is simple and obvious: A type pack $\overline{T}$ is a subtype of another $\overline{U}$ if each type in $\overline{T}$ is a subtype of the corresponding type from $\overline{U}$.  There is no subtyping relation at all between finite packs of inequal length.

Function subtyping is standard except that we do a subtype test between type packs.

Luau does not directly support unions of type packs, but it is nevertheless very convenient to think about a variadic type pack as an infinite union of finite packs.  If the pack $ \overline{T} $ is equal to $...T$ (that is, 0 or more $T$s), then

$$
\overline{T} = () \space | \space (T) \space | \space (T, T) \space | \space (T, T, T) \space | \space ...
$$
...
$$
\cfrac
{\overline{T} <: \overline{A} \space or \space \overline{T} <: \overline{B}}
{\overline{T} <: \overline{A} \space | \space \overline{B}}
$$
...
$$
\cfrac
{\overline{C} <: \overline{A}, \overline{B} <: \overline{D}}
{\overline{A} \rightarrow \overline{B} <: \overline{C} \rightarrow \overline{D}}
$$

And we have an easy way to reason about tests like the following:

$$
\begin{aligned}
(A, B, ...C) \rightarrow () &<: (D, E) \rightarrow () \\
(D, E) &<: (A, B, ...C) \\
(D, E) &<: (A, B) \space | \space (A, B, C) \space | \space (A, B, C, C) \space | \space (A, B, C, C, C) \space | \space ... \\
D <: A \space &and \space E <: B
\end{aligned}
$$
